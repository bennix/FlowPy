...
- 点击“运行工作流”后，画布会按真实的拓扑执行顺序回放：当前节点和正在传递数据的连线高亮，未激活分支显示 skipped。
- 运行完成后点击任意连线，可在右侧查看该输出端口传入下游前的结构化数据快照；不需要在日志里猜测中间值。
- 左下“扩展库”提供 pandas、requests、numpy、openpyxl、pyyaml 的明确安装入口。安装后可在 Python 或 ImportModule 节点中导入；自定义业务节点使用 Module 封装函数并保存为流程示例。
