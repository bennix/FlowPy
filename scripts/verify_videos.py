...
for plan in plans:
    ...
    entries.append({'slug': slug, 'name': plan['name'], 'duration': timeline['duration'],
                    'steps': len(timeline['steps']), 'verified': True})
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>FlowPy 从空画布搭建 · 26 个实例</title>
...
<h1>FlowPy · 从空画布开始</h1><p>26 个独立例子。每个从零节点、零连线开始，选择节点、填写参数、连接端口，并实际运行。</p>
