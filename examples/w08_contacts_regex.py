...
# FlowPy source sha256: 4e67083b0873ace1ca7f24fca8fe16449dfaab7abb2cf3d759dacd979f2e3d65
import re
import inspect
import asyncio
    ...
    try:
        ...
        if trace is not None:
            if isinstance(trace, list):
                trace.append({'node': 'text', 'status': 'skipped' if results['text'] is None else 'completed'})
            else:
        ...
        if trace is not None:
            if isinstance(trace, list):
                trace.append({'node': 'emails', 'status': 'skipped' if results['emails'] is None else 'completed'})
            else:
        ...
        if trace is not None:
            if isinstance(trace, list):
                trace.append({'node': 'phones', 'status': 'skipped' if results['phones'] is None else 'completed'})
            else:
        ...
        if trace is not None:
            if isinstance(trace, list):
                trace.append({'node': 'combine', 'status': 'skipped' if results['combine'] is None else 'completed'})
            else:
