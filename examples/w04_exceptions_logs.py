...
# FlowPy source sha256: 932405b52a6935afd6c780b3ae125fef3d36b55008c39f4a0706100473fc483c
import re
import inspect
import asyncio
    ...
    try:
        ...
        if trace is not None:
            if isinstance(trace, list):
                trace.append({'node': 'pairs', 'status': 'skipped' if results['pairs'] is None else 'completed'})
            else:
        ...
        if trace is not None:
            if isinstance(trace, list):
                trace.append({'node': 'function', 'status': 'skipped' if results['function'] is None else 'completed'})
            else:
